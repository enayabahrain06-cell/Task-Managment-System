...
class SettingsController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('admin.settings', compact('settings', 'stats', 'supportUsers', 'aboutPageTeamMembers', 'aboutPageRealStats'));
    }
    ...
    public function toggleElement(Request $request)
    {
        ...
        if (in_array($request->action, ['add', 'remove'])) {
            ...
            if ($request->action === 'add') {
                if (! in_array($request->key, $extras)) {
                    $extras[] = $request->key;
                }
            } else {
        ...
        if ($request->action === 'hide') {
            if (! in_array($request->key, $hidden)) {
                $hidden[] = $request->key;
            }
        } else {
    ...
    /** Whitelisted About Page settings a single inline edit is allowed to touch,
     *  keyed to their validation rule. Shared by the live-editor endpoint below —
     *  never accept an arbitrary setting key from the client. */
    private function aboutPageFieldRules(): array
    {
        $rules = [
            'about_page_intro' => 'nullable|string|max:200',
            'about_page_who_text' => 'nullable|string|max:600',
            'about_page_quote_text' => 'nullable|string|max:200',
            ...
            'about_page_cta_enabled' => 'nullable|boolean',
            'about_page_services_heading' => 'nullable|string|max:60',
            'about_page_stat3_value' => 'nullable|integer|min:0|max:999',
            'about_page_stat4_value' => 'nullable|integer|min:0|max:100',
            'about_page_mission_hidden' => 'nullable|boolean',
            'about_page_vision_hidden' => 'nullable|boolean',
            'about_page_show_who' => 'nullable|boolean',
            'about_page_show_stats' => 'nullable|boolean',
            'about_page_show_mission_vision' => 'nullable|boolean',
            'about_page_show_values' => 'nullable|boolean',
            'about_page_show_team' => 'nullable|boolean',
            'about_page_show_journey' => 'nullable|boolean',
            'about_page_show_services' => 'nullable|boolean',
            'about_page_show_gallery' => 'nullable|boolean',
            'about_page_bg_overlay' => 'nullable|integer|min:0|max:80',
        ];
        foreach ([1, 2, 3, 4, 5, 6] as $i) {
            $rules["about_page_value{$i}_title"] = 'nullable|string|max:40';
            $rules["about_page_value{$i}_desc"] = 'nullable|string|max:160';
            $rules["about_page_value{$i}_hidden"] = 'nullable|boolean';
            $rules["about_page_service{$i}_title"] = 'nullable|string|max:60';
            $rules["about_page_service{$i}_desc"] = 'nullable|string|max:160';
            $rules["about_page_service{$i}_hidden"] = 'nullable|boolean';
        }
        foreach ([1, 2, 3, 4] as $i) {
            $rules["about_page_stat{$i}_hidden"] = 'nullable|boolean';
            $rules["about_page_journey{$i}_hidden"] = 'nullable|boolean';
        }

        return $rules;
    }

    private function aboutPageBooleanFields(): array
    {
        return array_keys(array_filter(
            $this->aboutPageFieldRules(),
            fn ($rule) => str_contains($rule, 'boolean')
        ));
    }

    /** Single-field autosave used by the Developer Mode live editor on the public /about page. */
    public function updateAboutPageField(Request $request)
    {
        $fieldRules = $this->aboutPageFieldRules();

        $request->validate([
            'key' => ['required', 'string', 'in:' . implode(',', array_keys($fieldRules))],
        ]);

        $key = $request->input('key');
        $request->validate(['value' => $fieldRules[$key]]);

        $isBoolean = in_array($key, $this->aboutPageBooleanFields(), true);
        $value = $isBoolean
            ? (filter_var($request->input('value'), FILTER_VALIDATE_BOOLEAN) ? '1' : '0')
            : (string) $request->input('value', '');

        Setting::set($key, $value);

        AuditLogger::log('settings.updated', null, "Live-edited About page field: {$key}", ['section' => 'about_page', 'key' => $key]);

        return response()->json(['success' => true]);
    }

    /** Hero background upload/remove for the Developer Mode live editor on /about. */
    public function updateAboutPageHeroBg(Request $request)
    {
        $request->validate([
            'about_page_bg_image' => 'nullable|file|mimes:png,jpg,jpeg,webp,mp4,webm,mov,m4v|max:20480',
            'remove' => 'nullable|boolean',
        ]);

        $old = Setting::get('about_page_bg_image');

        if ($request->boolean('remove')) {
            if ($old) {
                Storage::disk('public')->delete($old);
            }
            Setting::set('about_page_bg_image', '');

            AuditLogger::log('settings.updated', null, 'Removed About page hero background', ['section' => 'about_page']);

            return response()->json(['success' => true, 'url' => null]);
        }

        if (! $request->hasFile('about_page_bg_image')) {
        ...
        $path = $request->file('about_page_bg_image')->store('branding', 'public');
        Setting::set('about_page_bg_image', $path);

        AuditLogger::log('settings.updated', null, 'Updated About page hero background', ['section' => 'about_page']);

        return response()->json([
            'success'  => true,
            'url'      => Storage::url($path),
            'is_video' => (bool) preg_match('/\.(mp4|webm|mov|m4v)$/i', $path),
        ]);
    }
    ...
    public function updateAboutPage(Request $request)
    {
        $rules = [
            ...
            'about_page_mission' => 'nullable|string|max:400',
            'about_page_vision' => 'nullable|string|max:400',
            'about_page_bg_image' => 'nullable|file|mimes:png,jpg,jpeg,webp,mp4,webm,mov,m4v',
            'about_page_bg_overlay' => 'nullable|integer|min:0|max:80',
            'about_page_quote_text' => 'nullable|string|max:200',
            'about_page_stat3_value' => 'nullable|integer|min:0|max:999',
            'about_page_stat4_value' => 'nullable|integer|min:0|max:100',
        ...
        foreach ([
            'about_page_show_who', 'about_page_show_stats', 'about_page_show_mission_vision',
            'about_page_show_values', 'about_page_show_team', 'about_page_show_journey',
            'about_page_show_services', 'about_page_show_gallery',
        ] as $section) {
            Setting::set($section, $request->has($section) ? '1' : '0');
        }

        // Per-card hide flags — hiding keeps the saved text, it just stops rendering
        foreach ([1, 2, 3, 4] as $i) {
            Setting::set("about_page_stat{$i}_hidden", $request->has("about_page_stat{$i}_hidden") ? '1' : '0');
            Setting::set("about_page_journey{$i}_hidden", $request->has("about_page_journey{$i}_hidden") ? '1' : '0');
        }
        Setting::set('about_page_mission_hidden', $request->has('about_page_mission_hidden') ? '1' : '0');
        Setting::set('about_page_vision_hidden', $request->has('about_page_vision_hidden') ? '1' : '0');
        ...
        Setting::set('about_page_bg_overlay', (int) $request->input('about_page_bg_overlay', 0));
        Setting::set('about_page_quote_text', $request->input('about_page_quote_text', ''));
        Setting::set('about_page_stat3_value', $request->input('about_page_stat3_value', ''));
        Setting::set('about_page_stat4_value', $request->input('about_page_stat4_value', ''));
        ...
        if ($request->input('remove_about_page_bg_image') === '1') {
            $old = Setting::get('about_page_bg_image');
            if ($old) {
                Storage::disk('public')->delete($old);
            }
        ...
        foreach ([1, 2, 3, 4, 5, 6] as $i) {
            ...
            Setting::set("about_page_service{$i}_hidden", $request->has("about_page_service{$i}_hidden") ? '1' : '0');
            ...
            Setting::set("about_page_value{$i}_hidden", $request->has("about_page_value{$i}_hidden") ? '1' : '0');
        }
        foreach ([1, 2, 3, 4, 5] as $i) {
    ...
    public function moveLoginFrame(Request $request)
    {
        ...
        if (array_key_exists('x', $data)) {
            Setting::set("login_frame{$data['slot']}_x", (string) round($data['x'], 2));
        }
        if (array_key_exists('y', $data)) {
            Setting::set("login_frame{$data['slot']}_y", (string) round($data['y'], 2));
        }
        if (array_key_exists('scale', $data)) {
            Setting::set("login_frame{$data['slot']}_scale", (string) round($data['scale'], 3));
        }
    ...
    public function testMail(Request $request)
    {
        ...
        try {
            ...
            $email = (new Email)
                ->from(new Address($request->from_address, $request->from_name))
                ->to($request->to)
    ...
    public function testStorageOmv(Request $request)
    {
        ...
        if ($protocol === 'webdav') {
            ...
            try {
                $resp = Http::withBasicAuth($username, $password)
                    ->timeout(8)
    ...
    public function migrateLocalToNas(Request $request)
    {
        ...
        TaskComment::whereNotNull('file_path')->whereNull('nas_path')
            ...
            ->chunk(50, function ($rows) use ($nas, &$success, &$failed, &$skipped) {
                foreach ($rows as $comment) {
                    ...
                    if (! Storage::disk('public')->exists($comment->file_path)) {
                        $skipped++;
        ...
        if ($failed > 0) {
            $msg .= " {$failed} failed (check NAS connection).";
        }
        if ($skipped > 0) {
            $msg .= " {$skipped} skipped (file missing from disk or no linked task).";
        }
    ...
    public function recoverNasPaths(Request $request)
    {
        ...
        if ($notFound > 0) {
            $msg .= " {$notFound} could not be found on NAS (files were never uploaded or are in an unknown location).";
        }
    ...
    private function sendUltramsg(string $token, string $phone, string $body): array
    {
        ...
        if (! $instanceId) {
            return ['ok' => false, 'message' => 'UltraMsg instance ID not set.'];
        }
    ...
    private function sendTwilio(string $token, string $phone, string $body): array
    {
        ...
        if (! $accountSid || ! $fromNumber) {
            return ['ok' => false, 'message' => 'Twilio Account SID / From Number not set.'];
        }
    ...
    private function sendMeta(string $token, string $phone, string $body): array
    {
        ...
        if (! $phoneNumberId) {
            return ['ok' => false, 'message' => 'Meta Phone Number ID not set.'];
        }
    ...
    private function imageToBase64(string $storagePath): string
    {
        ...
        $mime = match ($ext) {
            'png' => 'png', 'gif' => 'gif', 'webp' => 'webp', default => 'jpeg'
        };
    ...
    public function restoreUsers(Request $request)
    {
        ...
        while (($row = fgetcsv($handle)) !== false) {
            ...
            if ($existing) {
                ...
                User::create(['name' => $data['name'] ?? 'User', 'email' => $email, 'role' => $role, 'password' => bcrypt(Str::random(16))]);
                $created++;
            }
        }
        fclose($handle);
    ...
    private function featureToggle(string $key): JsonResponse
    {
    ...
    public function toggleGanttChart()
    {
        return $this->featureToggle('show_gantt_chart');
    }

    public function toggleExcelExport()
    {
        return $this->featureToggle('show_excel_export');
    }

    public function toggleWorkloadView()
    {
        return $this->featureToggle('show_workload_view');
    }

    public function toggleTaskDependencies()
    {
        return $this->featureToggle('show_task_dependencies');
    }

    public function toggleRecurringTasks()
    {
        return $this->featureToggle('show_recurring_tasks');
    }

    public function toggleTimeTracking()
    {
        return $this->featureToggle('show_time_tracking');
    }

    public function toggleTaskTemplates()
    {
        return $this->featureToggle('show_task_templates');
    }
}
