...
class TaskApprovalController extends Controller
{
    ...
    public function sendWhatsappMediaToCustomer(Request $request)
    {
        ...
        if (!in_array($provider, ['ultramsg', 'openwa'])) {
            return response()->json(['ok' => false, 'message' => 'File attachments are only supported with the UltraMsg and Open-WA providers.'], 422);
        }
        ...
        try {
            if ($provider === 'openwa') {
                return response()->json($this->sendOpenwaMedia($token, $digits, $request->file_url, $request->filename, $request->caption ?? '', $isImage));
            }
    ...
    private function sendOpenwaMedia(string $token, string $phone, string $url, string $filename, string $caption, bool $isImage): array
    {
        $baseUrl = Setting::get('wa_openwa_url', '');
        if (!$baseUrl) return ['ok' => false, 'message' => 'Open-WA API base URL not set.'];
        $session = Setting::get('wa_openwa_session', '');
        if (!$session) return ['ok' => false, 'message' => 'Open-WA session ID not set.'];

        $chatId = $phone.'@c.us';
        $response = $isImage
            ? Http::withHeaders(['X-API-Key' => $token])
                ->post(rtrim($baseUrl, '/')."/api/sessions/{$session}/messages/send-image", [
                    'chatId' => $chatId, 'url' => $url, 'caption' => $caption,
                ])
            : Http::withHeaders(['X-API-Key' => $token])
                ->post(rtrim($baseUrl, '/')."/api/sessions/{$session}/messages/send-document", [
                    'chatId' => $chatId, 'url' => $url, 'filename' => $filename,
                ]);

        return $response->successful()
            ? ['ok' => true, 'message' => 'File sent via Open-WA.']
            : ['ok' => false, 'message' => $this->waErrMsg($response->json('message'), 'Open-WA error.')];
    }
    ...
    private function dispatchWhatsapp(string $digits, string $body): array
    {
        ...
        return match ($provider) {
            ...
            'openwa'   => $this->sendOpenwa($token, $digits, $body),
    ...
    private function sendOpenwa(string $token, string $phone, string $body): array
    {
        ...
        return $response->successful()
            ? ['ok' => true, 'message' => 'Sent via Open-WA.']
            : ['ok' => false, 'message' => $this->waErrMsg($response->json('message'), 'Open-WA error.')];
    }
