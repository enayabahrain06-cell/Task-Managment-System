...
class DashboardController extends Controller
{
    public function analyticsTasks(Request $request)
    {
        ...
        $tasks = $query->get()->map(function ($task) use ($priorityMeta) {
    ...
    public function socialPosts(Request $request)
    {
        ...
        $query = TaskSocialPost::with(['task:id,title', 'user:id,name'])
            ->latest();
    ...
    public function refresh()
    {
        ...
        $taskOverview = [
            ...
            'reopened' => TaskLog::where('action', 'status_updated_reopened')->distinct('task_id')->count('task_id'),
            'reassigned' => TaskLog::whereIn('action', ['task_reassigned', 'task_transferred'])->distinct('task_id')->count('task_id'),
            'revision_requested' => Task::where('status', 'revision_requested')->count(),
        ];

        $statusCounts = Task::selectRaw('status, count(*) as c')->groupBy('status')->pluck('c', 'status');
    ...
    public function index()
    {
        ...
        // --- Per-status counts (mobile Pipeline card) ---
        $statusCounts = Task::selectRaw('status, count(*) as c')->groupBy('status')->pluck('c', 'status');
        ...
        foreach ($monthTasks as $dateKey => $tasks) {
            ...
            if ($tasks->where('status', 'in_progress')->count()) {
                $colors[] = '#F59E0B';
            }
            if ($tasks->where('status', 'pending')->count()) {
                $colors[] = '#6366F1';
            }
            if ($tasks->where('deadline', '<', now())->where('status', '!=', 'completed')->count()) {
                $colors[] = '#EF4444';
            }
        ...
        return view('admin.dashboard', compact(
            ...
            'taskOverview', 'completionRate', 'onTimeRate', 'reviewCycles', 'statusCounts',
