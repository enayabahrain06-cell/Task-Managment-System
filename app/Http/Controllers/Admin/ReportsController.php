...
{
    public function index(Request $request, ReportPdfService $reportPdfService)
    {
        ...
        $approvalSpeedTasks = Task::with(['customer', 'project.customer', 'assignee'])
            ...
            ->when($userId && !$isAdminManagerFilter, fn($q) => $q->where(fn($uq) =>
                $uq->where('assigned_to', $userId)
                   ->orWhere('social_assigned_to', $userId)
                   ->orWhereExists(fn($sub) => $sub->selectRaw('1')->from('task_assignees')
                       ->whereColumn('task_assignees.task_id', 'tasks.id')
                       ->where('task_assignees.user_id', $userId))
            ))
        ...
        $customerStats = $reportPdfService->customerDistStats($from, null, $doneStatuses, $customerId);
    ...
    public function summaryData(Request $request, ReportPdfService $reportPdfService)
    {
        ...
        return response()->json([
            ...
            'customerStatsMonthly' => $customerStatsMonthly,
            'taskList'       => $taskList,
