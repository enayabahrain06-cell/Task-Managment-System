...
class TaskController extends Controller
{
    public function index(Request $request)
    {
        ...
        $inProgressStatuses = ['in_progress', 'paused', 'submitted', 'pending_customer'];
        ...
        if ($request->filled('project')) {
            $base->where('project_id', $request->project);
        }
        ...
        match ($filter) {
            ...
            'overdue' => $base->where('deadline', '<', now())->whereNotIn('status', $completedStatuses),
            default => null,
        };
        ...
        $counts = [
            ...
            'overdue' => $userQuery()->where('deadline', '<', now())->whereNotIn('status', $completedStatuses)->count(),
        ];
    ...
    public function startTimer(Request $request, Task $task)
    {
        ...
        if (Setting::get('show_task_dependencies', '1') === '1'
            && $task->status === 'viewed'
    ...
    public function logTimeManually(Request $request, Task $task)
    {
        ...
        if (Setting::get('show_time_tracking', '1') !== '1') {
            abort(404);
        }
        ...
        $startedAt = Carbon::parse($logDate.' 09:00:00');
    ...
    public function requestDeadlineExtension(Request $request, Task $task)
    {
        ...
        TaskLog::create([
            ...
            'note' => 'Deadline extension requested until '.Carbon::parse($request->requested_deadline)->format('M d, Y').'. Reason: '.$request->reason,
            'metadata' => [
                'requested_deadline' => $request->requested_deadline,
                'reason' => $request->reason,
            ],
