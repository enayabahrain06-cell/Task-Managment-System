...
class NasService
{
    ...
    public function copyToNasReference(Task $task, string $localPath, string $originalFilename): ?string
    {
        ...
        // Note: this is a secondary/mirror copy — its result is not persisted per-record,
        // so it must never delete the local file. Only copyToNas() (whose return value is
        // stored as the record's nas_path) may do that; otherwise a failed primary copy
        // combined with a successful mirror copy silently orphans the file with no
        // recorded location at all.
        if (!$ok) {
    ...
    public function copyToNasDeliverable(Task $task, string $localPath, string $originalFilename): ?string
    {
        ...
        // Secondary/mirror copy — see comment in copyToNasReference(). Must not delete local.
        if (!$ok) {
