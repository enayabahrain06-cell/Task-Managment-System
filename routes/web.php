...
Route::middleware(['auth', MfaMiddleware::class])->group(function () {
    ...
    Route::post('/user/presence', function (Request $request) {
        ...
        $user->save();
        MqttService::presenceUpdate($user->id, $status);
    ...
    Route::get('/online-users', function () {
        ...
        $users = User::where('last_seen_at', '>=', now()->subMinutes(3))
    ...
    Route::get('/mobile/tasks', [MobileTaskController::class, 'index'])->name('mobile.tasks');
    Route::get('/mobile/tasks/{task}', [MobileTaskController::class, 'show'])->name('mobile.tasks.show');
    Route::get('/alerts', [NotificationsController::class, 'index'])->name('alerts.index');
    ...
    Route::get('/submissions/{submission}/file', function (TaskSubmission $submission) {
        ...
        if (Storage::disk('public')->exists($submission->file_path)) {
            ...
            return $inline
                ...
                : Storage::disk('public')->download($submission->file_path, $submission->original_filename ?? 'file');
        }
        if ($submission->nas_path) {
            return app(NasService::class)->downloadFromNas($submission->nas_path, $submission->original_filename ?? 'file', $inline);
        }
...
Route::middleware([AdminMiddleware::class, MfaMiddleware::class])->prefix('admin')->name('admin.')->group(function () {
    ...
    Route::post('settings/hide-features-tab', [AdminSettingsController::class, 'toggleHideFeaturesTab'])->name('settings.hide-features-tab');
    ...
    Route::post('settings/backup/restore-chunk', [AdminSettingsController::class, 'restoreChunkUpload'])->name('settings.backup.restore.chunk');
    Route::post('settings/backup/restore-chunk/finalize', [AdminSettingsController::class, 'restoreChunkFinalize'])->name('settings.backup.restore.chunk.finalize');
    ...
    Route::get('attachments/{attachment}/download', function (ProjectAttachment $attachment) {
        ...
        if (Storage::disk('public')->exists($attachment->path)) {
            ...
            return $inline
                ? response()->file($fullPath)
                : Storage::disk('public')->download($attachment->path, $attachment->name);
        }
        if ($attachment->nas_path) {
            return app(NasService::class)->downloadFromNas($attachment->nas_path, $attachment->name, $inline);
        }
        abort(404, 'File not found.');
    })->name('attachments.download');

    // Task attachment upload & delete
    Route::post('tasks/{task}/attachments', [TaskController::class, 'addAttachment'])->name('tasks.attachments.add');
    ...
    Route::get('submissions/{submission}/download', function (TaskSubmission $submission) {
        abort_unless($submission->file_path, 404);
        $inline = request()->boolean('inline');
        if (Storage::disk('public')->exists($submission->file_path)) {
            $fullPath = storage_path('app/public/'.$submission->file_path);
    ...
    Route::get('task-comments/{comment}/file', function (TaskComment $comment) {
        $authUser = auth()->user();
        ...
        if ($comment->files && $idx >= 0 && isset($comment->files[$idx])) {
            ...
            if ($nasPath) {
                return app(NasService::class)->downloadFromNas($nasPath, $filename, $inline);
            }
            abort(404, 'File not found.');
        }
        ...
        if (Storage::disk('public')->exists($comment->file_path)) {
            ...
            return $inline
                ? response()->file($fullPath)
                : Storage::disk('public')->download($comment->file_path, $filename);
        }
        if ($comment->nas_path) {
            return app(NasService::class)->downloadFromNas($comment->nas_path, $filename, $inline);
        }
        abort(404, 'File not found.');
...
Route::middleware([UserMiddleware::class, MfaMiddleware::class])->prefix('user')->name('user.')->group(function () {
    ...
    Route::put('/domains/{domain}', [UserDomainsController::class, 'update'])->name('domains.update');
    ...
    Route::get('/attachments/{attachment}/download', function (ProjectAttachment $attachment) {
        $userId = auth()->id();
        $allowed = Task::where('project_id', $attachment->project_id)
