...
@section('content')
<style>
.utask-filter-grid { display:grid; grid-template-columns:repeat(4,1fr); gap:14px; margin-bottom:24px; }
    ...
    <div class="divide-y divide-gray-50">
        ...
        <a href="{{ route('user.tasks.show', $task) }}"
           class="flex items-center gap-4 px-5 py-3.5 hover:bg-gray-50/70 transition group
            ...
            @if($task->deadline)
            <span class="utask-deadline-col text-xs flex-shrink-0 {{ $isOverdue ? 'text-red-500 font-semibold' : 'text-gray-400' }}">
                {{ $task->deadline->format(config('app.date_format', 'M d, Y')) }}
            </span>
            @else
            <span class="utask-deadline-col text-xs text-gray-300 flex-shrink-0">No deadline</span>
            @endif
