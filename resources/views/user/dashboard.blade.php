...
<div x-data="{
                    ...
                    <div style="padding:14px 18px 12px;border-bottom:1px solid #F3F4F6;display:flex;align-items:center;justify-content:space-between;">
                        <div style="display:flex;align-items:center;gap:8px;">
                            <div style="width:28px;height:28px;border-radius:8px;background:#EEF2FF;display:flex;align-items:center;justify-content:center;">
                                <i class="fas fa-layer-group" style="font-size:12px;color:#4F46E5;"></i>
                            </div>
                            <div>
                                <h3 style="font-size:13px;font-weight:700;color:#111827;margin:0;">My Licenses</h3>
                                <p style="font-size:10px;color:#9CA3AF;margin:0;">{{ $myLicenses->count() }} tool{{ $myLicenses->count()>1?'s':'' }} assigned</p>
                            </div>
                        </div>
                        <a href="{{ route('user.licenses.index') }}"
                           style="font-size:11px;color:#4F46E5;background:#EEF2FF;border:none;cursor:pointer;font-weight:600;padding:4px 10px;border-radius:7px;text-decoration:none;">
                            View all
                        </a>
                    </div>

                    @if($licExpiring->count())
                    <div style="margin:10px 12px 0;background:#FFF7ED;border:1px solid #FED7AA;border-radius:8px;padding:8px 12px;display:flex;align-items:center;gap:8px;">
                        <i class="fas fa-triangle-exclamation" style="color:#EA580C;font-size:11px;flex-shrink:0;"></i>
                        <span style="font-size:11.5px;color:#92400E;font-weight:600;">
                            {{ $licExpiring->count() }} license{{ $licExpiring->count()>1?'s':'' }} expiring within 7 days
                        </span>
                    </div>
                    @endif

                    <div style="padding:6px 0 4px;">
                        @foreach($myLicenses->take(4) as $lic)
                        @php
                        ...
                        <a href="{{ route('user.licenses.index') }}"
                           ...
                           onmouseover="this.style.background='#F9FAFB'" onmouseout="this.style.background='transparent'">
                            <div style="width:32px;height:32px;border-radius:9px;background:{{ $lcc['bg'] }};display:flex;align-items:center;justify-content:center;flex-shrink:0;">
                                <i class="fas fa-{{ $lic->category === 'design' ? 'pen-nib' : ($lic->category === 'development' ? 'code' : ($lic->category === 'communication' ? 'comment-dots' : ($lic->category === 'marketing' ? 'bullhorn' : ($lic->category === 'security' ? 'shield-halved' : ($lic->category === 'finance' ? 'chart-line' : 'layer-group'))))) }}"
                                   style="font-size:12px;color:{{ $lcc['color'] }};"></i>
                            </div>
                            <div style="flex:1;min-width:0;">
                                <p style="font-size:12px;font-weight:600;color:#111827;margin:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">{{ $lic->name }}</p>
                                <p style="font-size:10px;color:#9CA3AF;margin:1px 0 0;">{{ $lic->vendor ?? \App\Models\Subscription::categoryOptions()[$lic->category] ?? 'Tool' }}</p>
                            </div>
                            @if($ldays !== null)
                                @if($ldays < 0)
                                <span style="font-size:10px;font-weight:700;color:#DC2626;background:#FEE2E2;padding:2px 7px;border-radius:10px;white-space:nowrap;">Expired</span>
                                @elseif($ldays <= 7)
                                <span style="font-size:10px;font-weight:700;color:#EA580C;background:#FFF7ED;padding:2px 7px;border-radius:10px;white-space:nowrap;">{{ $ldays }}d</span>
                                @elseif($ldays <= 30)
                                <span style="font-size:10px;font-weight:600;color:#D97706;background:#FEF3C7;padding:2px 7px;border-radius:10px;white-space:nowrap;">{{ $ldays }}d</span>
                                @else
                                <span style="font-size:10px;color:#9CA3AF;white-space:nowrap;">{{ $lic->renewal_date?->format('M y') }}</span>
                                @endif
                            @endif
                        </a>
                        @endforeach

                        @if($myLicenses->count() > 4)
                        <div style="padding:6px 18px 8px;">
                            <a href="{{ route('user.licenses.index') }}"
                               style="font-size:11px;color:#6B7280;text-decoration:none;">
                                +{{ $myLicenses->count() - 4 }} more license{{ $myLicenses->count()-4>1?'s':'' }}
                            </a>
                        </div>
                        @endif
                    </div>
    ...
    <div x-show="expiringModal" style="position:fixed;inset:0;z-index:9100;overflow-y:auto;" x-cloak>
        ...
        <div style="background:#fff;border-radius:16px;width:480px;max-width:100%;max-height:85vh;display:flex;flex-direction:column;overflow:hidden;box-shadow:0 24px 64px rgba(0,0,0,.2);">
            <div style="padding:20px 24px;border-bottom:1px solid #F3F4F6;display:flex;align-items:center;justify-content:space-between;flex-shrink:0;">
                <div style="display:flex;align-items:center;gap:10px;">
                    ...
                    <div>
                        <h3 style="font-size:16px;font-weight:700;color:#111827;margin:0;">Your Domains Are Expiring Soon</h3>
