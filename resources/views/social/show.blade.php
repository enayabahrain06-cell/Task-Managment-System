...
$latestSubFiles = $latestSub ? $task->submissions->where('version', $latestSub->version) : collect();
...
<div class="social-show-layout">
    ...
    <div style="display:flex;flex-direction:column;gap:20px;">
        ...
        <div class="socshow-card" style="background:#fff;border-radius:18px;border:1px solid #E5E7EB;box-shadow:0 2px 12px rgba(0,0,0,.06);overflow:hidden;">
            ...
            {{-- Submission files --}}
            @if($latestSub)
            <div style="padding:18px 24px;" x-data="{ subLbOpen:false, subLbUrl:'', subLbName:'' }">
                <p style="font-size:10px;font-weight:700;color:#9CA3AF;text-transform:uppercase;letter-spacing:.06em;margin:0 0 12px;">
                    ...
                    @if($latestSubFiles->count() > 1)
                    <span style="font-size:10px;background:#F3F4F6;color:#6B7280;padding:2px 8px;border-radius:20px;margin-left:4px;font-weight:700;">{{ $latestSubFiles->count() }} files</span>
                    @endif
                </p>
                @foreach($latestSubFiles->pluck('note')->filter()->unique() as $subNote)
                @php
                    $subNoteText = trim(strip_tags(preg_replace('/<\/(div|p)>|<br\s*\/?>/i', "\n", $subNote)));
                @endphp
                <div style="background:#F8FAFC;border-radius:10px;padding:12px 16px;margin-bottom:14px;border:1px solid #EEF2F8;">
                    <p dir="auto" style="font-size:13px;color:#374151;margin:0;line-height:1.65;font-style:italic;white-space:pre-line;">"{{ $subNoteText }}"</p>
                </div>
                @endforeach
                <div style="display:flex;flex-direction:column;gap:12px;">
                    @foreach($latestSubFiles as $subFile)
                    @if($subFile->file_path)
                    @php
                        $fname = $subFile->original_filename ?? '';
                        $fext  = strtolower(pathinfo($fname, PATHINFO_EXTENSION));
                        $furl  = $subFile->fileUrl();
                        $isImg = in_array($fext, ['jpg','jpeg','png','gif','webp','svg']);
                        $isVid = in_array($fext, ['mp4','mov','avi','mkv','webm']);
                    @endphp
                    @if($isImg)
                    <div>
                        <button type="button" @click="subLbUrl='{{ $furl }}'; subLbName='{{ $fname }}'; subLbOpen=true"
                                style="display:block;width:100%;padding:0;background:none;cursor:zoom-in;border-radius:12px;overflow:hidden;border:1.5px solid #DDE3F5;max-height:420px;">
                ...
                <template x-teleport="body">
                    <div x-show="subLbOpen" x-cloak @keydown.escape.window="subLbOpen=false" style="position:fixed;inset:0;z-index:9999;">
