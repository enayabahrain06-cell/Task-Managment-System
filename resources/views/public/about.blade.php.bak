...
<div class="relative z-10 grain"
    ...
    <section id="devHeroSection" class="relative overflow-hidden"
         ...
         @else
         style="background: radial-gradient(60% 50% at 15% 10%, color-mix(in srgb, var(--brand) 16%, transparent), transparent 70%),
        ...
        <div class="wave-row relative z-10 flex items-end justify-center gap-6 max-w-5xl mx-auto px-6 pt-16 pb-14 flex-wrap">
            ...
            <div class="wave-card relative flex flex-col items-center" style="margin-top:-{{ $card['lift'] }}px; animation-delay:{{ 0.08 * $i }}s;">
    ...
    <section class="relative max-w-5xl mx-auto px-6 pb-20 md:pb-28 {{ $showFullGridOnDesktop ? '' : 'lg:hidden' }} {{ $devEditOn ? 'dev-hideable' : '' }}" @if($devEditOn) data-hidden="{{ $sectionOn['team'] ? '0' : '1' }}" @endif>
        ...
        @else
            <div class="reveal grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-5">
                ...
                <div class="bg-white rounded-[22px] p-6 text-center shadow-[0_4px_20px_rgba(99,102,241,0.08)] border border-black/[0.03] hover:-translate-y-1 hover:shadow-[0_12px_28px_rgba(99,102,241,0.16)] transition" style="transition-delay:{{ $i * 30 }}ms;">
                    @if($member->avatar)
...
(function () {
    ...
    if (heroSection) {
        ...
        heroSection.addEventListener('dragover', function (e) {
            e.preventDefault();
            if (heroDropOverlay) heroDropOverlay.style.display = 'flex';
        });
