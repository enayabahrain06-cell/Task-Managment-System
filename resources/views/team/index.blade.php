...
<div class="mb-4 bg-red-50 border border-red-200 text-red-700 text-sm px-4 py-3 rounded-xl">
    <div class="flex items-center gap-2 font-semibold mb-1"><i class="fa fa-circle-exclamation"></i> Could not save user — please fix the following errors:</div>
    <ul class="list-disc list-inside space-y-0.5">
        @foreach($errors->userEdit->all() as $error)
        <li>{{ $error }}</li>
        @endforeach
    </ul>
</div>
    ...
    $permGroups = [
        'Tasks & Work' => [
            ...
            'perms' => [
                ...
                'delete_tasks'         => ['icon' => 'fa-trash',               'label' => 'Delete Tasks',         'desc' => 'Move tasks to recycle bin'],
                'view_trash'           => ['icon' => 'fa-trash-can',           'label' => 'Recycle Bin',          'desc' => 'Access & restore deleted tasks'],
        ...
        'Projects & Team' => [
            ...
            'perms' => [
                ...
                'delete_projects' => ['icon' => 'fa-trash',            'label' => 'Delete Projects',        'desc' => 'Permanently delete projects'],
    ...
    <div style="position:relative;width:100%;max-width:600px;background:#fff;border-radius:20px;box-shadow:0 32px 80px rgba(0,0,0,0.22);display:flex;flex-direction:column;max-height:calc(100vh - 32px);overflow:hidden;">
        ...
        <div style="overflow-y:auto;flex:1;padding:20px 24px;">
            <form id="edit-user-form" method="POST" :action="'/admin/users/' + userId"
                  ...
                  @submit.prevent="pwdMismatch = !!(editPwd && editPwd !== editPwdConfirm); if(!pwdMismatch){ saving=true; $el.submit(); }">
                @csrf
                @method('PUT')
                ...
                <div style="border:1.5px solid #F3F4F6;border-radius:12px;overflow:hidden;margin-bottom:20px;">
                    ...
                    <div class="team-form-grid-2" style="padding:14px;display:grid;grid-template-columns:1fr 1fr;gap:12px;" x-data="{s1:false,s2:false}">
                        <div style="position:relative;">
                            <input :type="s1?'text':'password'" name="password" x-model="editPwd"
                                   ...
                                   :style="pwdMismatch ? 'border-color:#EF4444;' : ''">
                        ...
                        <div style="position:relative;">
                            <input :type="s2?'text':'password'" name="password_confirmation" x-model="editPwdConfirm"
                    ...
                    <div x-show="pwdMismatch" style="padding:0 14px 12px;">
                        <p style="font-size:11px;color:#EF4444;margin:0;display:flex;align-items:center;gap:5px;">
                            <i class="fas fa-circle-exclamation" style="font-size:11px;"></i> Passwords do not match.
                        </p>
                    </div>
                </div>
...
@php
    ...
    $addPermGroups = [
        'Tasks & Work'    => ['icon'=>'fa-square-check','color'=>'#6366F1','bg'=>'#EEF2FF','perms'=>['view_tasks'=>'View Tasks','submit_work'=>'Submit Work','manage_tasks'=>'Manage Tasks','delete_tasks'=>'Delete Tasks','view_trash'=>'Recycle Bin','approve_tasks'=>'Approve Submissions','view_activity_log'=>'Activity Log','view_version_history'=>'Version History','view_comments'=>'Comments']],
        'Projects & Team' => ['icon'=>'fa-diagram-project','color'=>'#10B981','bg'=>'#ECFDF5','perms'=>['view_projects'=>'View Projects','manage_projects'=>'Manage Projects','delete_projects'=>'Delete Projects','view_team_tasks'=>'Team Tasks','view_team'=>'Team Directory']],
        ...
        <div style="overflow-y:auto;flex:1;padding:20px 24px;">
            <form id="add-user-form" method="POST" action="{{ route('admin.users.store') }}"
                ...
                <div style="border:1.5px solid #F3F4F6;border-radius:12px;overflow:hidden;margin-bottom:16px;">
                    ...
                    <p style="font-size:11px;color:#9CA3AF;margin:10px 14px 0;">
                        <i class="fa fa-circle-info" style="margin-right:4px;"></i>{{ $passwordRequirementText }}
                    </p>
                    <div class="team-form-grid-2" style="padding:14px;display:grid;grid-template-columns:1fr 1fr;gap:12px;" x-data="{s1:false,s2:false}">
                        <div style="position:relative;">
                            <input :type="s1?'text':'password'" name="password" required placeholder="Min. {{ $minPasswordLength }} characters"
                                   style="width:100%;padding:9px 36px 9px 12px;border:1.5px solid #E5E7EB;border-radius:10px;font-size:13px;background:#F9FAFB;color:#111827;outline:none;box-sizing:border-box;">
                            <button type="button" x-on:click="s1=!s1" style="position:absolute;right:10px;top:50%;transform:translateY(-50%);background:none;border:none;cursor:pointer;color:#9CA3AF;">
                                <i :class="s1?'fa fa-eye-slash':'fa fa-eye'" style="font-size:13px;"></i>
                            </button>
                    ...
                    <p x-show="pwdError" x-text="pwdError" style="color:#DC2626;font-size:12px;font-weight:600;margin:0 14px 12px;"></p>
                </div>
...
function addUserModal() {
    ...
    return {
        ...
        handleSubmit(e) {
            ...
            const strong  = {{ $requireStrongPassword ? 'true' : 'false' }};

            this.pwdError = '';
            if (pwd.length < minLen) {
                this.pwdError = `Password must be at least ${minLen} characters.`;
            } else if (strong && !(/[a-z]/.test(pwd) && /[A-Z]/.test(pwd) && /\d/.test(pwd) && /[^A-Za-z0-9]/.test(pwd))) {
                this.pwdError = 'Password must contain an uppercase letter, a lowercase letter, a number, and a special character.';
            } else if (pwd !== confirm) {
