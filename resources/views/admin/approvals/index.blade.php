            ...
            <div style="padding:14px 24px;border-top:1px solid #F0F2F8;display:flex;align-items:center;gap:8px;flex-wrap:wrap;background:#FAFBFF;flex-shrink:0;">
                ...
                <template x-if="qvTask?.customer_phone">
                    <div style="display:inline-flex;align-items:center;gap:6px;">
                        ...
                        @unless($hideWaWeb)
                        <button type="button"
                                @click="quickWhatsAppWeb(qvTask.customer_phone, qvTask.customer_name, qvTask.title, qvTask.submission_url)"
                                style="display:inline-flex;align-items:center;gap:5px;padding:0 12px;height:36px;background:#fff;color:#25D366;border:1.5px solid #25D366;border-radius:10px;font-size:12px;font-weight:600;cursor:pointer;white-space:nowrap;transition:opacity .15s;"
                                onmouseover="this.style.opacity='.8'" onmouseout="this.style.opacity='1'"
                                title="Send preview via WhatsApp Web">
                    ...
                    <div x-show="approvalNotifyWhatsapp" x-transition style="margin-bottom:10px;">
                        ...
                        <div style="display:flex;align-items:center;flex-wrap:wrap;gap:8px;">
                            ...
                            <button type="button" @click="openCustomerWhatsApp()"
                                    style="display:inline-flex;align-items:center;gap:6px;padding:7px 14px;background:#fff;color:#25D366;border:1.5px solid #25D366;border-radius:9px;font-size:12px;font-weight:600;cursor:pointer;transition:opacity .15s;"
                                    onmouseover="this.style.opacity='.8'" onmouseout="this.style.opacity='1'">
            ...
            <td style="text-align:right;white-space:nowrap;" onclick="event.stopPropagation()">
                <div style="display:inline-flex;align-items:center;gap:5px;">
                    ...
                    @php
                        ...
                        $lwMsg = str_replace(
                            ['{customer_name}', '{task_title}', '{design_link}', '{company}'],
                            [$lwName, $task->title, $lwFile ?: '', $company],
                            $waPreviewTpl
                        );
                    @endphp
                    ...
                    <div data-wa-phone="{{ $lwPhone }}" data-wa-msg="{{ $lwMsg }}" data-wa-file="{{ $lwFile }}"
                        ...
                        <div x-show="waOpen" x-cloak x-transition @click.outside="waOpen=false"
                             :style="`position:fixed;top:${waTop}px;right:${waRight}px;z-index:9999;background:#fff;border-radius:16px;width:256px;box-shadow:0 20px 50px rgba(0,0,0,.18);border:1px solid #D1FAE5;overflow:hidden;`">
                            ...
                            <div style="padding:14px 16px;">
                                ...
                                @unless($hideWaWeb)
                                <button type="button" @click="openWeb()"
                                        style="width:100%;display:flex;align-items:center;justify-content:center;gap:7px;padding:9px;background:#fff;color:#25D366;border:1.5px solid #25D366;border-radius:9px;font-size:13px;font-weight:600;cursor:pointer;margin-top:7px;transition:opacity .15s;"
                                        onmouseover="this.style.opacity='.8'" onmouseout="this.style.opacity='1'">
                                    <i class="fab fa-whatsapp" style="font-size:14px;"></i>
                                    Send via WhatsApp Web ↗
                                </button>
...
{{-- Search / filter bar --}}
<form method="GET" action="{{ route('admin.approvals.index') }}" style="margin-bottom:16px;">
    <input type="hidden" name="tab" value="decide_later">
    <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;">
        <div style="position:relative;flex:1;min-width:180px;">
            <i class="fas fa-search" style="position:absolute;left:10px;top:50%;transform:translateY(-50%);font-size:11px;color:#9CA3AF;pointer-events:none;"></i>
            <input type="text" name="dlsearch" value="{{ $dlSearch ?? '' }}" placeholder="Search tasks…"
                   style="width:100%;padding:7px 10px 7px 30px;border:1.5px solid #E5E7EB;border-radius:8px;font-size:12px;color:#374151;outline:none;box-sizing:border-box;"
                   onfocus="this.style.borderColor='#FBB040'" onblur="this.style.borderColor='#E5E7EB'">
        </div>
        @if($dlAssignees->isNotEmpty())
        <select name="dlassignee" style="padding:7px 10px;border:1.5px solid #E5E7EB;border-radius:8px;font-size:12px;color:#374151;outline:none;background:#fff;"
                onfocus="this.style.borderColor='#FBB040'" onblur="this.style.borderColor='#E5E7EB'">
            <option value="">All assignees</option>
            @foreach($dlAssignees as $da)
            <option value="{{ $da->id }}" {{ ($dlAssignee ?? '') == $da->id ? 'selected' : '' }}>{{ $da->name }}</option>
            @endforeach
        </select>
        @endif
        <button type="submit" style="padding:7px 14px;background:linear-gradient(135deg,#F59E0B,#D97706);color:#fff;border:none;border-radius:8px;font-size:12px;font-weight:600;cursor:pointer;white-space:nowrap;">
            <i class="fas fa-filter" style="font-size:10px;margin-right:4px;"></i> Filter
        </button>
        @if($dlSearch || $dlAssignee)
        <a href="{{ route('admin.approvals.index') }}?tab=decide_later"
           style="padding:7px 12px;background:#F3F4F6;color:#6B7280;border-radius:8px;font-size:12px;font-weight:600;text-decoration:none;white-space:nowrap;">
            <i class="fas fa-times" style="font-size:10px;margin-right:3px;"></i> Clear
        </a>
        @endif
    </div>
</form>
...
<div x-data="{
    selected: [],
    bulkUserId: '',
    pageIds: @json($decideLaterTasks->pluck('id')),
    get allSelected() { return this.selected.length === this.pageIds.length && this.pageIds.length > 0; },
    toggleAll(checked) { this.selected = checked ? [...this.pageIds] : []; },
    bulkSubmit(action) {
        if (!this.selected.length) return;
        if (action === 'not_needed' && !confirm('Mark ' + this.selected.length + ' task(s) as social not needed?')) return;
        if (action === 'assign' && !this.bulkUserId) { alert('Please select a handler first.'); return; }
        const form = document.createElement('form');
        form.method = 'POST';
        form.action = '{{ route('admin.approvals.bulk-decide-later') }}';
        const csrf = Object.assign(document.createElement('input'), {type:'hidden', name:'_token', value:'{{ csrf_token() }}'});
        const act  = Object.assign(document.createElement('input'), {type:'hidden', name:'action', value: action});
        form.appendChild(csrf);
        form.appendChild(act);
        if (action === 'assign') {
            const uid = Object.assign(document.createElement('input'), {type:'hidden', name:'social_user_id', value: this.bulkUserId});
            form.appendChild(uid);
        }
        this.selected.forEach(id => {
            const inp = Object.assign(document.createElement('input'), {type:'hidden', name:'task_ids[]', value: id});
...
<div x-show="selected.length > 0" x-transition
     style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;background:linear-gradient(135deg,#FFFBEB,#FEF3C7);border:1.5px solid #FDE68A;border-radius:12px;padding:10px 14px;margin-bottom:12px;">
    <span style="font-size:12px;font-weight:700;color:#92400E;">
        <i class="fas fa-check-square" style="margin-right:5px;color:#D97706;"></i>
        <span x-text="selected.length"></span> task(s) selected
    </span>
    <div style="flex:1;"></div>
    <button @click="bulkSubmit('not_needed')"
            style="display:inline-flex;align-items:center;gap:5px;padding:6px 13px;background:#FEE2E2;color:#DC2626;border:1.5px solid #FECACA;border-radius:8px;font-size:12px;font-weight:600;cursor:pointer;white-space:nowrap;"
            onmouseover="this.style.background='#FECACA'" onmouseout="this.style.background='#FEE2E2'">
        <i class="fas fa-ban" style="font-size:10px;"></i> Mark as Not Needed
    </button>
    <div style="display:flex;align-items:center;gap:5px;">
        <select x-model="bulkUserId"
                style="padding:6px 10px;border:1.5px solid #E5E7EB;border-radius:8px;font-size:12px;color:#374151;outline:none;background:#fff;"
                onfocus="this.style.borderColor='#FBB040'" onblur="this.style.borderColor='#E5E7EB'">
        ...
        <button @click="bulkSubmit('assign')"
                style="display:inline-flex;align-items:center;gap:5px;padding:6px 13px;background:linear-gradient(135deg,#F59E0B,#D97706);color:#fff;border:none;border-radius:8px;font-size:12px;font-weight:600;cursor:pointer;white-space:nowrap;box-shadow:0 2px 6px rgba(245,158,11,.25);"
                onmouseover="this.style.opacity='.88'" onmouseout="this.style.opacity='1'">
            <i class="fas fa-share-alt" style="font-size:10px;"></i> Assign Selected
        </button>
    </div>
</div>
...
<div class="tbl-scroll">
    <table class="hist-table" style="table-layout:auto;">
        <thead>
            <tr>
                <th style="width:44px;padding:11px 8px 11px 16px;">
        ...
        <tbody x-data="{ expanded: false }">
            <tr :style="selected.includes({{ $dl->id }}) ? 'background:#FFFBEB;' : ''">
                <td style="padding:12px 8px 12px 16px;width:44px;">
                    <div style="display:flex;align-items:center;gap:6px;">
                        <input type="checkbox" value="{{ $dl->id }}" x-model="selected"
                               style="width:14px;height:14px;cursor:pointer;accent-color:#D97706;">
                        <button @click="expanded = !expanded"
                ...
                <td @click="expanded = !expanded" style="cursor:pointer;">
                    ...
                    <div style="display:flex;align-items:center;gap:6px;margin-top:3px;flex-wrap:wrap;">
                        ...
                        @if($dl->social_platforms && count($dl->social_platforms))
                            @foreach($dl->social_platforms as $plt)
                            <span style="font-size:10px;font-weight:600;padding:1px 7px;border-radius:20px;background:#EDE9FE;color:#5B21B6;">{{ ucfirst($plt) }}</span>
                            @endforeach
                        @endif
                    </div>
                </td>
                ...
                <td style="white-space:nowrap;">
                    ...
                    <form method="POST" action="{{ route('admin.tasks.social.required', $dl->id) }}" style="margin:0;">
                        ...
                        <button type="button"
                                onclick="if(confirm('Mark \'{{ addslashes($dl->title) }}\' as social not needed?')) this.closest('form').submit();"
                            ...
                            <i class="fas fa-ban" style="font-size:10px;"></i> No Social
                        </button>
                    </form>
                    <div x-data="{ menuOpen: false, dTop: 0, dRight: 0 }" @click.outside="menuOpen=false" @scroll.window="menuOpen=false" @keydown.escape.window="menuOpen=false">
                        ...
                        <div x-show="menuOpen"
                            ...
                            <button @click="menuOpen=false; expanded=true"
                                    style="display:flex;align-items:center;gap:9px;width:100%;padding:11px 15px;background:linear-gradient(135deg,#EFF6FF,#DBEAFE);border:none;border-bottom:1px solid #BFDBFE;font-size:12px;font-weight:700;color:#1E40AF;cursor:pointer;text-align:left;"
                                    onmouseover="this.style.background='linear-gradient(135deg,#DBEAFE,#BFDBFE)'" onmouseout="this.style.background='linear-gradient(135deg,#EFF6FF,#DBEAFE)'">
                                <i class="fas fa-share-alt" style="font-size:12px;width:14px;text-align:center;color:#3B82F6;"></i> Assign for Social
                            </button>
            ...
            <tr x-show="expanded" x-collapse style="background:rgb(239,246,255);">
                <td colspan="2"></td>
                <td colspan="3" style="padding:0 16px 16px;">
                    <div style="padding-top:14px;border-top:1px solid #BFDBFE;">
                        <p style="font-size:10px;font-weight:700;color:#9CA3AF;text-transform:uppercase;letter-spacing:.05em;margin:0 0 10px;">
                            <i class="fas fa-share-alt" style="margin-right:4px;color:#3B82F6;"></i>Assign for Social Media Posting
                        </p>
                        <form method="POST" action="{{ route('admin.tasks.social.assign', $dl->id) }}" style="display:flex;flex-direction:column;gap:8px;max-width:480px;">
                            ...
                            <div style="display:flex;gap:6px;align-items:center;">
                                ...
                                <button type="submit"
                                        style="padding:7px 16px;background:linear-gradient(135deg,#3B82F6,#2563EB);color:#fff;border:none;border-radius:8px;font-size:12px;font-weight:700;cursor:pointer;white-space:nowrap;box-shadow:0 2px 8px rgba(59,130,246,.25);transition:opacity .15s;"
                                        onmouseover="this.style.opacity='.88'" onmouseout="this.style.opacity='1'">
...
<div style="display:flex;align-items:center;justify-content:center;gap:6px;margin-top:20px;flex-wrap:wrap;">
    ...
    @else
    <a href="{{ $decideLaterTasks->appends(['tab'=>'decide_later','dlsearch'=>$dlSearch,'dlassignee'=>$dlAssignee])->previousPageUrl() }}" style="padding:6px 12px;border-radius:8px;font-size:12px;font-weight:600;background:#F3F4F6;color:#374151;text-decoration:none;">‹ Prev</a>
    @endif
    @foreach($decideLaterTasks->appends(['tab'=>'decide_later','dlsearch'=>$dlSearch,'dlassignee'=>$dlAssignee])->getUrlRange(1, $decideLaterTasks->lastPage()) as $page => $url)
    ...
    <a href="{{ $decideLaterTasks->appends(['tab'=>'decide_later','dlsearch'=>$dlSearch,'dlassignee'=>$dlAssignee])->nextPageUrl() }}" style="padding:6px 12px;border-radius:8px;font-size:12px;font-weight:600;background:#F3F4F6;color:#374151;text-decoration:none;">Next ›</a>
    @else
...
function approvalPage() {
    return {
        ...
        async quickWhatsApp(phone, name, title, submissionUrl) {
            ...
            const tpl  = @json($waPreviewTpl);
            const base = tpl
                .replace('{customer_name}', name)
                .replace('{task_title}', title)
                .replace('{design_link}', submissionUrl ?? '');
        ...
        quickWhatsAppWeb(phone, name, title, submissionUrl) {
            if (!phone) { alert('No WhatsApp number on file for this customer.'); return; }
        ...
        buildWhatsAppMessage() {
            ...
            const link   = task?.submission_url ?? '';
            const tpl    = @json($waPreviewTpl);
            const base   = tpl
                .replace('{customer_name}', name)
                .replace('{task_title}', title)
                .replace('{design_link}', link);
