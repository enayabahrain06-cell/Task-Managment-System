...
@media print {
    ...
    .app-main    { display:block !important; height:auto !important; overflow:visible !important; margin-left:0 !important; padding:0 !important; }
    main.app-content { overflow:visible !important; height:auto !important; flex:none !important; padding:0 !important; margin:0 !important; animation:none !important; }

    /* Suppress browser link-URL annotations (e.g. Firefox/Chrome default print behaviour) */
...
.cust-show-split { display:grid; grid-template-columns:1fr 2fr; gap:20px; align-items:stretch; }
...
}" x-init="editableMsg = sendMsg; $watch('sendMode', () => { editableMsg = sendMsg; }); $watch('selectedFile', () => { editableMsg = sendMsg; }); $watch('sendProject', () => { editableMsg = sendMsg; });" @keydown.escape.window="close()" @open-customer-task.window="openTask($event.detail)" style="width:100%;min-width:0;">
...
<div style="position:fixed;inset:0;z-index:9999;display:flex;align-items:center;justify-content:center;padding:16px;"
    ...
    <div style="display:flex;align-items:center;gap:12px;margin-bottom:24px;">
        ...
        <div x-data="{ exportOpen: false }" style="position:relative;">
            ...
            <div x-show="exportOpen" x-transition
                ...
                <div style="height:1px;background:#F3F4F6;"></div>
                <button @click="exportOpen=false; window.printCustomerShow()"
                        style="width:100%;padding:10px 16px;background:none;border:none;text-align:left;font-size:13px;font-weight:500;color:#374151;cursor:pointer;display:flex;align-items:center;gap:9px;"
                        onmouseover="this.style.background='#F9FAFB'" onmouseout="this.style.background=''">
                    <i class="fas fa-print" style="font-size:13px;color:#6366F1;width:16px;text-align:center;"></i> Print
                </button>
                <div style="height:1px;background:#F3F4F6;"></div>
                <button @click="exportOpen=false; window.exportCustomerShowPDF()"
                        style="width:100%;padding:10px 16px;background:none;border:none;text-align:left;font-size:13px;font-weight:500;color:#374151;cursor:pointer;display:flex;align-items:center;gap:9px;"
                        onmouseover="this.style.background='#F9FAFB'" onmouseout="this.style.background=''">
                    <i class="fas fa-file-pdf" style="font-size:13px;color:#DC2626;width:16px;text-align:center;"></i> Export PDF
                </button>
            </div>
        </div>
    </div>
    ...
    <div class="cust-show-split" style="gap:20px;align-items:stretch;">
        ...
        <div style="background:#fff;border-radius:14px;border:1px solid #F0F0F0;box-shadow:0 1px 4px rgba(0,0,0,.04);padding:22px;display:flex;flex-direction:column;align-self:stretch;height:100%;box-sizing:border-box;">
            ...
            <div style="padding-top:16px;border-top:1px solid #F3F4F6;margin-top:auto;">
        ...
        <div style="display:flex;flex-direction:column;gap:10px;">
            ...
            <div style="background:#fff;border-radius:14px;border:1px solid #F0F0F0;box-shadow:0 1px 4px rgba(0,0,0,.04);padding:12px 20px;display:flex;align-items:center;justify-content:space-between;">
                <h2 style="font-size:13px;font-weight:700;color:#374151;text-transform:uppercase;letter-spacing:.05em;margin:0;">
                    Projects <span style="font-weight:500;color:#9CA3AF;">(0)</span>
                    <span style="font-size:11px;font-weight:400;color:#D1D5DB;text-transform:none;letter-spacing:0;margin-left:6px;">— none linked yet</span>
                </h2>
                <a href="{{ route('admin.projects.create') }}"
                   style="font-size:11px;font-weight:600;color:#4F46E5;background:#EEF2FF;padding:4px 10px;border-radius:6px;text-decoration:none;flex-shrink:0;">
                    + New Project
                </a>
            </div>
            @else
...
/* ══════════════════════════════════════════════════════════
   CUSTOMER SHOW — Branded Print & Export PDF
══════════════════════════════════════════════════════════ */
@php
$showLogo = '';
if (!empty($appSettings['logo_path'])) {
    ...
    if (file_exists($lp)) {
        ...
        $showLogo = 'data:' . $mime . ';base64,' . base64_encode(file_get_contents($lp));
    }
}
@endphp
function buildCustomerShowHTML(immediate) {
    ...
    var custInit = custName.charAt(0).toUpperCase();
    ...
    @php
        ...
        if ($customer->logo) {
            ...
            if (file_exists($clp)) {
                $cmime = mime_content_type($clp) ?: 'image/jpeg';
                $showCustLogoSrc = 'data:' . $cmime . ';base64,' . base64_encode(file_get_contents($clp));
            }
        }
    @endphp
    var custLogoSrc = '{{ $showCustLogoSrc }}';

    var allTasks  = @json($csvRows);
    var projects  = @json($exportProjects);
    var statPending   = {{ $statPending }};
    var statActive    = {{ $statActive }};
    var statDone      = {{ $statDone }};
    var statOverdue   = {{ $statOverdue }};
    var totalTasks    = allTasks.length;
    var completionRate = totalTasks > 0 ? Math.round(statDone / totalTasks * 100) : 0;
    ...
    + '.cav { width:42px; height:42px; border-radius:11px; display:flex; align-items:center; justify-content:center; font-size:17px; font-weight:700; color:#fff; flex-shrink:0; -webkit-print-color-adjust:exact; print-color-adjust:exact; }'
    + '.cav-img { width:42px; height:42px; border-radius:11px; object-fit:cover; flex-shrink:0; }'
    + '.cinfo-name { font-size:15px; font-weight:800; color:#111827; }'
    + '.cinfo-co   { font-size:11px; color:#6B7280; margin-top:2px; }'
    + '.cquick { display:flex; gap:16px; margin-left:auto; }'
    + '.cq-item { text-align:center; }'
    + '.cq-val { font-size:18px; font-weight:800; color:#4F46E5; line-height:1; }'
    + '.cq-lbl { font-size:9px; color:#9CA3AF; margin-top:2px; }'
    + '.meta { display:flex; background:#F8FAFC; padding:10px 32px; border-bottom:1px solid #E5E7EB; }'
    ...
    + '.mi-val { font-size:11px; font-weight:600; color:#374151; margin-top:2px; }'
    + '.content { padding:20px 32px 26px; }'
    + '.sh { display:flex; align-items:center; gap:10px; margin:20px 0 12px; padding-bottom:8px; border-bottom:1.5px solid #F3F4F6; }'
    ...
    + '.kgrid { display:grid; grid-template-columns:repeat(4,1fr); gap:10px; margin-bottom:4px; }'
    ...
    + '.k-val { font-size:26px; font-weight:900; color:#111827; line-height:1; letter-spacing:-1px; }'
    ...
    + 'td { padding:8px 12px; border-bottom:1px solid #F3F4F6; font-size:11.5px; color:#374151; vertical-align:middle; }'
    ...
    + '.foot  { margin:4px 32px 0; padding:12px 0; border-top:2px solid #4F46E5; display:flex; justify-content:space-between; align-items:center; -webkit-print-color-adjust:exact; print-color-adjust:exact; }'
    ...
    + '@@media print { .pbar{display:none !important;} body{background:#fff !important;} .doc{max-width:none;} *{-webkit-print-color-adjust:exact !important;print-color-adjust:exact !important;} tr{page-break-inside:avoid;} @@page{size:A4 landscape;margin:0;} }'
    ;

    // ── Status badge map ─────────────────────────────────
    var statusBg    = { draft:'#F3F4F6', assigned:'#EEF2FF', viewed:'#E0F2FE', in_progress:'#FFF7ED', submitted:'#F5F3FF', revision_requested:'#FEE2E2', approved:'#ECFDF5', delivered:'#ECFDF5', archived:'#F3F4F6' };
    var statusColor = { draft:'#6B7280', assigned:'#4F46E5', viewed:'#0369A1', in_progress:'#EA580C', submitted:'#7C3AED', revision_requested:'#DC2626', approved:'#16A34A', delivered:'#16A34A', archived:'#9CA3AF' };
    var statusLabel = { draft:'Draft', assigned:'Assigned', viewed:'Viewed', in_progress:'In Progress', submitted:'In Review', revision_requested:'Revision', approved:'Approved', delivered:'Delivered', archived:'Archived' };
    var projBg    = { active:'#EEF2FF', completed:'#ECFDF5', on_hold:'#FEF3C7', cancelled:'#FEE2E2' };
    var projColor = { active:'#4F46E5', completed:'#16A34A', on_hold:'#D97706', cancelled:'#DC2626' };
    ...
    var custAvatarEl = custLogoSrc
        ? '<img class="cav-img" src="' + custLogoSrc + '">'
        : '<div class="cav" style="background:linear-gradient(135deg,#6366F1,#8B5CF6);">' + custInit + '</div>';
    ...
    // ── KPI cards ─────────────────────────────────────────
    var kpis = [
        { label:'Pending',    value:statPending,    color:'#6366F1', bg:'#EEF2FF', sub:'Draft / Assigned / Viewed' },
        { label:'In Progress',value:statActive,     color:'#EA580C', bg:'#FFF7ED', sub:'Active tasks' },
        { label:'Completed',  value:statDone,       color:'#16A34A', bg:'#ECFDF5', sub:completionRate + '% rate' },
        { label:'Overdue',    value:statOverdue,    color:'#DC2626', bg:'#FEF2F2', sub:'Past deadline' },
    ];
    ...
    if (projects.length > 0) {
        projHtml += '<div class="sh"><div class="sh-bar" style="background:#0EA5E9;"></div><div class="sh-text">Projects</div><span class="sh-pill">' + projects.length + '</span></div>';
        projHtml += '<div class="twrap"><table><thead><tr>'
                 + '<th style="width:28px;">#</th><th>Project</th><th style="text-align:center;">Tasks</th><th>Deadline</th><th>Status</th>'
                 + '</tr></thead><tbody>';
        projects.forEach(function(p, i) {
            var pb = projBg[p.status.toLowerCase().replace(' ','_')] || '#F3F4F6';
            var pc = projColor[p.status.toLowerCase().replace(' ','_')] || '#374151';
            projHtml += '<tr>'
                     + '<td style="color:#9CA3AF;font-size:10px;">' + (i+1) + '</td>'
                     + '<td class="td-n">' + p.name + '</td>'
                     + '<td style="text-align:center;">' + p.tasks_count + '</td>'
                     + '<td>' + (p.deadline || '—') + (p.overdue ? ' <span style="color:#DC2626;font-weight:700;">⚠</span>' : '') + '</td>'
                     + '<td><span class="rb" style="background:' + pb + ';color:' + pc + ';">' + p.status + '</span></td>'
                     + '</tr>';
        });
    ...
    // ── Tasks table ───────────────────────────────────────
    var tHtml = '<div class="sh"><div class="sh-bar" style="background:#4F46E5;"></div><div class="sh-text">All Tasks</div><span class="sh-pill">' + allTasks.length + ' tasks &mdash; ' + completionRate + '% done</span></div>';
    tHtml += '<div class="twrap"><table><thead><tr>'
          + '<th style="width:28px;">#</th>'
          + '<th>Task</th><th>Project</th><th>Assignee</th><th>Deadline</th><th>Status</th>'
          + '</tr></thead><tbody>';
    allTasks.forEach(function(t, i) {
        var sKey = t.status ? t.status.toLowerCase() : '';
        var sb   = statusBg[sKey]    || '#F3F4F6';
        var sc   = statusColor[sKey] || '#374151';
        var sl   = statusLabel[sKey] || t.status || '';
        var od   = t.deadline && !['approved','delivered','archived'].includes(sKey) && new Date(t.deadline) < new Date();
        tHtml += '<tr>'
              + '<td style="color:#9CA3AF;font-size:10px;">' + (i+1) + '</td>'
              + '<td class="td-n">' + t.title + '</td>'
              + '<td style="color:#6B7280;">' + (t.project || '—') + '</td>'
              + '<td>' + (t.assignee || '—') + '</td>'
              + '<td>' + (t.deadline ? t.deadline + (od ? ' <span style="color:#DC2626;font-weight:700;">⚠</span>' : '') : '—') + '</td>'
              + '<td><span class="rb" style="background:' + sb + ';color:' + sc + ';">' + sl + '</span></td>'
              + '</tr>';
    });
    ...
    var pbar = immediate ? '' :
        '<div class="pbar">'
      + '<div class="pbar-l"><h2>' + custName + (custCo ? ' &mdash; ' + custCo : '') + '</h2>'
      + '<p>' + allTasks.length + ' tasks &bull; Generated ' + dateStr + '</p></div>'
    ...
    return '<!DOCTYPE html><html lang="en"><head>'
         ...
         + '<title>' + custName + ' — Customer Report</title>'
         + '<style>' + css + '</style>'
         + printOnLoad
         + '</head><body>'
         + pbar
         ...
         +   custAvatarEl
         +   '<div><div class="cinfo-name">' + custName + '</div>'
         +     (custCo ? '<div class="cinfo-co">' + custCo + '</div>' : '') + '</div>'
         +   '<div class="cquick">'
         +     '<div class="cq-item"><div class="cq-val">' + totalTasks + '</div><div class="cq-lbl">Tasks</div></div>'
         +     '<div class="cq-item"><div class="cq-val" style="color:#16A34A;">' + completionRate + '%</div><div class="cq-lbl">Done</div></div>'
         +     '<div class="cq-item"><div class="cq-val" style="color:#DC2626;">' + statOverdue + '</div><div class="cq-lbl">Overdue</div></div>'
         +   '</div>'
         + '</div>'
         ...
         +   (custEmail ? '<div class="mi"><div class="mi-lbl">Email</div><div class="mi-val">' + custEmail + '</div></div>' : '')
         +   (custPhone ? '<div class="mi"><div class="mi-lbl">Phone</div><div class="mi-val">' + custPhone + '</div></div>' : '')
         +   '<div class="mi"><div class="mi-lbl">Prepared By</div><div class="mi-val">' + user + '</div></div>'
         + '</div>'

         /* Content */
         + '<div class="content">'
         +   '<div class="sh"><div class="sh-bar" style="background:#4F46E5;"></div><div class="sh-text">Overview</div></div>'
         +   kHtml
         +   projHtml
         +   tHtml
         + '</div>'
         ...
         /* Footer */
         + '<div class="foot">'
         +   '<div class="foot-brand">' + company + ' &mdash; ' + custName + '</div>'
...
window.printCustomerShow = function () {
    var win = window.open('', '_blank');
    if (win) { win.document.write(buildCustomerShowHTML(true)); win.document.close(); }
    else { alert('Pop-up blocked — please allow pop-ups for this site.'); }
};

window.exportCustomerShowPDF = function () {
    _pdfDownload(buildCustomerShowHTML, '{{ Str::slug($customer->name) }}-customer.pdf');
};
