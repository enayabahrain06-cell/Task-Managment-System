...
/* ── Status filter chips ── */
.sa-stat-chip { display:flex; align-items:center; gap:10px; padding:12px 18px; background:#fff; border:1.5px solid #E5E7EB; border-radius:12px; cursor:pointer; transition:all .15s; flex:1; min-width:110px; }
.sa-stat-chip:hover { border-color:#A5B4FC; box-shadow:0 2px 8px rgba(99,102,241,.1); }
.sa-stat-chip.active-filter { border-color:#4F46E5; box-shadow:0 0 0 3px rgba(99,102,241,.12); background:#FAFBFF; }
/* ── View toggle ── */
...
/* ── Cards — Swiss style ── */
.sa-card { background:#fff; border:1px solid #E2E4E9; border-radius:10px; overflow:hidden; display:flex; flex-direction:column; transition:box-shadow .18s; }
.sa-card:hover { box-shadow:0 4px 20px rgba(0,0,0,.09); }
...
<div style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:20px;">
    <div class="sa-stat-chip" :class="statusFilter==='all' ? 'active-filter' : ''" @click="statusFilter='all'">
        ...
        <div>
            <p style="font-size:20px;font-weight:800;color:#111827;margin:0;line-height:1;">{{ $stats['total'] }}</p>
            <p style="font-size:10px;color:#9CA3AF;margin:1px 0 0;font-weight:600;text-transform:uppercase;letter-spacing:.04em;">Total</p>
        </div>
    </div>
    <div class="sa-stat-chip" :class="statusFilter==='active' ? 'active-filter' : ''" @click="statusFilter='active'">
        ...
        <div>
            <p style="font-size:20px;font-weight:800;color:#16A34A;margin:0;line-height:1;">{{ $stats['active'] }}</p>
            <p style="font-size:10px;color:#9CA3AF;margin:1px 0 0;font-weight:600;text-transform:uppercase;letter-spacing:.04em;">Active</p>
        </div>
    </div>
    <div class="sa-stat-chip" :class="statusFilter==='inactive' ? 'active-filter' : ''" @click="statusFilter='inactive'">
        <div style="width:34px;height:34px;border-radius:10px;background:#F3F4F6;display:flex;align-items:center;justify-content:center;flex-shrink:0;">
            <i class="fas fa-moon" style="color:#6B7280;font-size:13px;"></i>
        </div>
        <div>
            <p style="font-size:20px;font-weight:800;color:#6B7280;margin:0;line-height:1;">{{ $stats['inactive'] }}</p>
            <p style="font-size:10px;color:#9CA3AF;margin:1px 0 0;font-weight:600;text-transform:uppercase;letter-spacing:.04em;">Inactive</p>
        </div>
    </div>
    <div class="sa-stat-chip" :class="statusFilter==='suspended' ? 'active-filter' : ''" @click="statusFilter='suspended'">
        ...
        <div>
            <p style="font-size:20px;font-weight:800;color:#DC2626;margin:0;line-height:1;">{{ $stats['suspended'] }}</p>
    ...
    <div class="sa-cards-grid" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(310px,1fr));gap:14px;margin-bottom:8px;">
        ...
        <div class="sa-card" style="animation-delay:{{ $idx * 0.035 }}s;border-left:4px solid {{ $pi['color'] }};"
            ...
            <div style="padding:16px 16px 14px;display:flex;align-items:flex-start;gap:12px;">
                {{-- Solid platform icon block --}}
                <div style="width:44px;height:44px;border-radius:8px;background:{{ $pi['color'] }};display:flex;align-items:center;justify-content:center;flex-shrink:0;">
                    <i class="fab {{ $pi['icon'] }}" style="font-size:20px;color:#fff;"></i>
                </div>
                {{-- Name + sub --}}
                <div style="flex:1;min-width:0;padding-top:1px;">
                    <p style="font-size:15px;font-weight:800;color:#111827;margin:0 0 3px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;letter-spacing:-.2px;">{{ $account->name }}</p>
                    @if($customer)
                    <a href="{{ route('admin.customers.show', $customer->id) }}"
                       style="font-size:11px;color:#6366F1;font-weight:600;text-decoration:none;display:inline-flex;align-items:center;gap:3px;">
                        <i class="fas fa-building" style="font-size:8px;"></i> {{ $customer->name }}
                    </a>
                    @elseif($account->username)
                    <span style="font-size:11px;color:#9CA3AF;font-family:monospace;">&#64;{{ $account->username }}</span>
                    @endif
                </div>
                {{-- Status: dot + ALL CAPS text only, no pill bg --}}
                <div style="display:flex;align-items:center;gap:5px;flex-shrink:0;padding-top:4px;">
                    <span style="width:6px;height:6px;border-radius:50%;background:{{ $stDot }};flex-shrink:0;"></span>
                    <span style="font-size:9px;font-weight:700;color:{{ $sc['color'] }};text-transform:uppercase;letter-spacing:.08em;">{{ $account->status }}</span>
                </div>
            </div>
