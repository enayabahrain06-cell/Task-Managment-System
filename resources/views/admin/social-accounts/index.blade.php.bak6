...
<style>
/* ── Credential helpers ── */
.sa-cred-val  { font-size:12px; font-weight:500; color:#111827; font-family:monospace; flex:1; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
.sa-cred-btn  { border:none; border-radius:6px; padding:3px 7px; cursor:pointer; font-size:10px; flex-shrink:0; transition:background .15s; }
...
/* ── Vault-style cred row ── */
.vc-row   { display:flex; align-items:center; gap:8px; }
.vc-icon  { width:20px; text-align:center; flex-shrink:0; }
.vc-label { font-size:10px; color:#9CA3AF; width:62px; flex-shrink:0; text-transform:uppercase; letter-spacing:.04em; }
...
<div x-show="view==='cards'">
    ...
    <div class="sa-cards-grid" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(310px,1fr));gap:14px;margin-bottom:8px;">
        ...
        <div class="sa-card" style="animation-delay:{{ $idx * 0.035 }}s;cursor:pointer;"
            ...
            {{-- ── Header: gradient + platform icon inline ── --}}
            <div style="background:linear-gradient(135deg,{{ $pi['color'] }}18,{{ $pi['color'] }}08);padding:16px 18px 13px;display:flex;align-items:flex-start;gap:13px;border-bottom:1.5px solid {{ $pi['color'] }}18;">
                <div style="width:50px;height:50px;border-radius:13px;background:{{ $pi['bg'] }};display:flex;align-items:center;justify-content:center;flex-shrink:0;box-shadow:0 2px 8px {{ $pi['color'] }}22;">
            ...
            {{-- ── Credentials ── --}}
            @if($hasCreds)
            <div style="padding:12px 18px;display:flex;flex-direction:column;gap:7px;border-bottom:1.5px solid #F3F4F6;">
                <p style="font-size:10px;font-weight:700;color:#4F46E5;text-transform:uppercase;letter-spacing:.05em;margin:0 0 2px;display:flex;align-items:center;gap:5px;">
                    <i class="fas fa-vault" style="font-size:9px;"></i> Credentials
                </p>
                @if($account->email)
                <div class="vc-row">
                    <div class="vc-icon"><i class="fas fa-envelope" style="font-size:10px;color:#A5B4FC;"></i></div>
                    <span class="vc-label">Email</span>
                    <span class="sa-cred-val">{{ $account->email }}</span>
                    <button class="sa-cred-btn sa-copy-btn" onclick="copyToClipboard('{{ addslashes($account->email) }}', this)" title="Copy"><i class="fas fa-copy"></i></button>
                </div>
                @endif
                @if($account->username)
                <div class="vc-row">
                    <div class="vc-icon"><i class="fas fa-at" style="font-size:10px;color:#C4B5FD;"></i></div>
                    <span class="vc-label">Handle</span>
                    <span class="sa-cred-val">{{ $account->username }}</span>
                    <button class="sa-cred-btn sa-copy-btn" onclick="copyToClipboard('{{ addslashes($account->username) }}', this)" title="Copy"><i class="fas fa-copy"></i></button>
                </div>
                @endif
                @if($account->password)
                <div class="vc-row">
                    <div class="vc-icon"><i class="fas fa-lock" style="font-size:10px;color:#FCA5A5;"></i></div>
                    <span class="vc-label">Password</span>
                    <span class="sa-cred-val" style="letter-spacing:.1em;">••••••••</span>
                ...
                <div class="vc-row">
                    <div class="vc-icon"><i class="fas fa-fingerprint" style="font-size:10px;color:#6EE7B7;"></i></div>
                    <span class="vc-label">Acct ID</span>
                    <span class="sa-cred-val">{{ $account->account_id }}</span>
                    <button class="sa-cred-btn sa-copy-btn" onclick="copyToClipboard('{{ addslashes($account->account_id) }}', this)" title="Copy"><i class="fas fa-copy"></i></button>
                </div>
                @endif
            </div>
            @endif
            ...
            {{-- ── Page URL ── --}}
            @if($account->page_url)
            <div style="padding:8px 18px;border-bottom:1.5px solid #F3F4F6;">
                <a href="{{ $account->page_url }}" target="_blank"
                   style="display:inline-flex;align-items:center;gap:6px;font-size:11px;font-weight:600;color:{{ $pi['color'] }};text-decoration:none;">
            ...
            <div style="padding:11px 18px;display:flex;align-items:center;justify-content:space-between;gap:8px;margin-top:auto;">
                ...
                <div style="display:flex;align-items:center;gap:7px;min-width:0;">
                    ...
                    <div style="display:flex;align-items:center;">
                        ...
                        <span title="{{ $au->name }}"
                              style="width:24px;height:24px;border-radius:50%;border:2px solid #fff;background:{{ $auc }};display:inline-flex;align-items:center;justify-content:center;font-size:9px;font-weight:700;color:#fff;{{ $loop->first ? '' : 'margin-left:-7px;' }}">
                            {{ strtoupper(substr($au->name, 0, 1)) }}
                        </span>
                        @endforeach
                        @if($assignedUsers->count() > 4)
                        <span style="width:24px;height:24px;border-radius:50%;border:2px solid #fff;background:#E5E7EB;display:inline-flex;align-items:center;justify-content:center;font-size:9px;font-weight:700;color:#6B7280;margin-left:-7px;">
                            +{{ $assignedUsers->count() - 4 }}
                        </span>
                        @endif
                    </div>
                    <span style="font-size:11px;color:#9CA3AF;font-weight:500;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:90px;">
                ...
                <div style="display:flex;gap:5px;flex-shrink:0;">
                    <button @click.stop="openEdit({
                            ...
                            style="display:inline-flex;align-items:center;gap:5px;padding:6px 13px;background:#EEF2FF;color:#4F46E5;border:none;border-radius:8px;font-size:11px;font-weight:700;cursor:pointer;transition:background .15s;"
                    ...
                    <button @click.stop="openDelete({{ $account->id }}, '{{ addslashes($account->name) }}')"
                            style="width:30px;height:30px;background:#FEF2F2;color:#DC2626;border:none;border-radius:8px;cursor:pointer;display:flex;align-items:center;justify-content:center;transition:background .15s;"
