            ...
            <div class="mob-card task-mob-card-pad" style="background:#fff;border-radius:14px;border:1px solid #F3F4F6;box-shadow:0 1px 4px rgba(0,0,0,.04);padding:24px;">
                ...
                <div style="display:flex;flex-direction:column;gap:8px;margin-bottom:12px;">
                    ...
                    <div style="display:flex;align-items:center;gap:8px;">
                        ...
                        @endphp
                        @if($canDelete && auth()->user()->hasPermission('delete_tasks'))
                        ...
                        <div class="task-mob-round task-mob-card-rowpad" style="background:#F9FAFB;border:1px solid #E5E7EB;border-radius:10px;padding:12px 14px;">
                            ...
                            @elseif($subFile->file_path)
                                @php
                                    ...
                                    $sfItem     = ['name'=>$subFile->original_filename ?? 'file','url'=>$sfUrl,'downloadUrl'=>$sfDlUrl,'previewUrl'=>$sfUrl,'isImage'=>$sfIsImage,'isVideo'=>$sfIsVideo,'isPdf'=>$sfIsPdf,'version'=>$subFile->version];
                                        ...
                                        <div style="background:#fff;border-radius:20px;width:100%;max-width:540px;max-height:80vh;display:flex;flex-direction:column;box-shadow:0 28px 80px rgba(0,0,0,.25);overflow:hidden;">
                                            ...
                                            <div style="padding:18px 22px 14px;border-bottom:1px solid {{ $sub->status === 'approved' ? '#D1FAE5' : '#FEE2E2' }};background:{{ $sub->status === 'approved' ? '#F0FDF4' : '#FFF8F8' }};display:flex;align-items:center;justify-content:space-between;flex-shrink:0;">
                                                <div style="display:flex;align-items:center;gap:10px;">
                                                    <div style="width:36px;height:36px;border-radius:10px;background:{{ $sub->status === 'approved' ? 'linear-gradient(135deg,#10B981,#059669)' : 'linear-gradient(135deg,#EF4444,#DC2626)' }};display:flex;align-items:center;justify-content:center;box-shadow:0 4px 10px {{ $sub->status === 'approved' ? 'rgba(16,185,129,.3)' : 'rgba(239,68,68,.3)' }};">
                                                        <i class="fa {{ $noteColor['icon'] }}" style="color:#fff;font-size:13px;"></i>
                                                    </div>
                                                    <div>
                ...
                <div x-show="open" x-cloak
                    ...
                    <div x-transition
                        ...
                        <div>
                            ...
                            <template x-if="sub.isPdf">
                                <div style="padding:16px 24px;border-bottom:1px solid #F3F4F6;background:#F9FAFB;">
                                    <iframe :src="sub.previewUrl || sub.url" style="width:100%;height:75vh;border:none;border-radius:10px;background:#fff;"></iframe>
                                </div>
                            </template>
                            <div style="padding:16px 24px;display:flex;gap:10px;justify-content:flex-end;">
                                <button @click="close()"
