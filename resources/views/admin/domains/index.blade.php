                ...
                <tr class="dom-row" style="border-bottom:1px solid #F3F4F6;">
                    ...
                    <td style="padding:14px 16px;text-align:right;">
                        <button onclick="openDomRowMenu(event, this, {{ $domain->id }}, {{ json_encode($domain->domain) }}, '{{ route('admin.domains.show', $domain->id) }}', {{ json_encode([
                            ...
                            'registeredAt'      => $domain->registered_at?->format('Y-m-d'),
...
// ── Row action dropdown ──────────────────────────────────────────────
function openDomRowMenu(evt, btn, id, name, viewUrl, ed) {
    evt.stopPropagation();
    const menu = document.getElementById('dom-row-action-menu');
...
function closeDomRowMenu() {
    const m = document.getElementById('dom-row-action-menu');
