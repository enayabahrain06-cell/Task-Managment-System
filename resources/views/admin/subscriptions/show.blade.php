    ...
    <div class="show-grid" style="display:grid;grid-template-columns:340px 1fr;gap:20px;align-items:start;">
        ...
        <div style="display:flex;flex-direction:column;gap:16px;">
            ...
            <div class="info-card" style="border-color:#E0E7FF;">
                ...
                @php $decrypted = $subscription->decrypted_password; @endphp
                <div>
                    <div class="info-label">Password / License Key</div>
                    <div style="display:flex;align-items:center;gap:8px;margin-top:4px;">
                        <code x-show="!showPassword" style="font-size:13px;color:#374151;background:#F3F4F6;padding:5px 10px;border-radius:6px;flex:1;letter-spacing:.15em;">••••••••••••</code>
                        <code x-show="showPassword" style="font-size:13px;color:#374151;background:#F3F4F6;padding:5px 10px;border-radius:6px;flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;word-break:break-all;">{{ $decrypted }}</code>
                        <button @click="showPassword = !showPassword" title="Show/hide"
                                style="width:28px;height:28px;border:none;border-radius:6px;background:#F3F4F6;cursor:pointer;display:flex;align-items:center;justify-content:center;color:#6B7280;flex-shrink:0;">
                            <i :class="showPassword ? 'fas fa-eye-slash' : 'fas fa-eye'" style="font-size:11px;"></i>
                        </button>
                        @if($decrypted)
                        <button onclick="copyToClipboard('{{ addslashes($decrypted) }}', this)" title="Copy"
                                style="width:28px;height:28px;border:none;border-radius:6px;background:#F3F4F6;cursor:pointer;display:flex;align-items:center;justify-content:center;color:#6B7280;flex-shrink:0;">
                            <i class="fas fa-copy" style="font-size:11px;"></i>
                        </button>
                        @endif
