...
function printRptSummSelection(sections) {
    ...
    if (sections && sections.taskDist && d && !d.hasUser && d.customerStats && d.customerStats.length) {
        ...
        var distRows = d.customerStats.slice().sort(function(a, b) { return b.total - a.total; });
        // Use sum of customer tasks as denominator so percentages add up to 100%
        var grandTotal = distRows.reduce(function(sum, c) { return sum + c.total; }, 0) || 0;
        ...
        bodyHtml += '<div>'
            ...
            + '<div>'
            + '<p style="font-size:11px;font-weight:700;color:#374151;text-transform:uppercase;letter-spacing:.06em;margin:0;">Task Distribution by Customer</p>'
            + '<p style="font-size:11px;color:#6B7280;margin:3px 0 0;">' + period + '</p>'
            + '</div>'
            + '<div style="text-align:right;">'
            + '<p style="font-size:11px;color:#9CA3AF;margin:0;">Total Tasks</p>'
            + '<p style="font-size:28px;font-weight:900;color:#4F46E5;margin:0;line-height:1.1;">' + grandTotal + '</p>'
            + '</div>'
            + '</div>'
            + '<table style="width:100%;border-collapse:collapse;">'
            + '<thead><tr style="background:#F8FAFC;">'
            + '<th style="text-align:left;padding:8px 10px;font-size:10px;font-weight:700;color:#6B7280;text-transform:uppercase;letter-spacing:.05em;border-bottom:1px solid #E5E7EB;">#</th>'
            + '<th style="text-align:left;padding:8px 10px;font-size:10px;font-weight:700;color:#6B7280;text-transform:uppercase;letter-spacing:.05em;border-bottom:1px solid #E5E7EB;">Customer</th>'
            + '<th style="text-align:center;padding:8px 10px;font-size:10px;font-weight:700;color:#6B7280;text-transform:uppercase;letter-spacing:.05em;border-bottom:1px solid #E5E7EB;">' + (sections.taskAsPct ? '% Share' : 'Tasks') + '</th>'
            + '<th style="padding:8px 10px;font-size:10px;font-weight:700;color:#6B7280;text-transform:uppercase;letter-spacing:.05em;border-bottom:1px solid #E5E7EB;min-width:120px;">Share</th>'
            + (sections.taskAsPct ? '' : '<th style="text-align:right;padding:8px 10px;font-size:10px;font-weight:700;color:#6B7280;text-transform:uppercase;letter-spacing:.05em;border-bottom:1px solid #E5E7EB;">% of Total</th>')
            + '</tr></thead><tbody>';

        distRows.forEach(function(c, i) {
            var pct     = grandTotal > 0 ? (c.total / grandTotal * 100) : 0;
            var pctStr  = Math.round(pct) + '%';
            var barW    = Math.round(pct);
            var rowBg   = i % 2 === 0 ? '#fff' : '#FAFAFA';
            var tasksCell = sections.taskAsPct
                ? '<td style="padding:9px 10px;font-size:13px;font-weight:800;color:#4F46E5;text-align:center;border-bottom:1px solid #F3F4F6;">' + pctStr + '</td>'
                : '<td style="padding:9px 10px;font-size:13px;font-weight:700;color:#4F46E5;text-align:center;border-bottom:1px solid #F3F4F6;">' + c.total + '</td>';
            bodyHtml += '<tr style="background:' + rowBg + ';">'
                + '<td style="padding:9px 10px;font-size:11px;color:#9CA3AF;border-bottom:1px solid #F3F4F6;">' + (i + 1) + '</td>'
                + '<td style="padding:9px 10px;font-size:13px;font-weight:600;color:#111827;border-bottom:1px solid #F3F4F6;">' + c.name + '</td>'
                + tasksCell
                ...
                + '</div>'
                + '</div></td>'
                + (sections.taskAsPct ? '' : '<td style="padding:9px 10px;font-size:13px;font-weight:800;color:#4F46E5;text-align:right;border-bottom:1px solid #F3F4F6;">' + pctStr + '</td>')
                + '</tr>';
        });
        ...
        bodyHtml += '<tr style="background:#EEF2FF;">'
            ...
            + (sections.taskAsPct
                ? '<td style="padding:10px 10px;font-size:13px;font-weight:800;color:#4F46E5;text-align:center;">100%</td>'
                : '<td style="padding:10px 10px;font-size:13px;font-weight:800;color:#4F46E5;text-align:center;">' + grandTotal + '</td>')
            + '<td></td>'
            + (sections.taskAsPct ? '' : '<td style="padding:10px 10px;font-size:13px;font-weight:800;color:#4F46E5;text-align:right;">100%</td>')
            + '</tr>';
    ...
    if ((!sections || sections.narrative !== false) && d) {
        ...
        var _custSum   = _hasCusts ? d.customerStats.reduce(function(s,c){return s+c.total;},0) : 0;
        ...
        if (_hasCusts) {
            var topC    = d.customerStats.slice().sort(function(a,b){return b.total-a.total;})[0];
            var topCpct = _custSum > 0 ? Math.round(topC.total / _custSum * 100) : 0;
            sentences.push('<strong>' + topC.name + '</strong> accounted for the highest task volume with <strong>' + topC.total + ' task' + (topC.total !== 1 ? 's' : '') + '</strong> (' + topCpct + '% of total workload).');
        }
        narrativeHtml =
